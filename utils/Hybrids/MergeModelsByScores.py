...
class MergeTwoModelsByScores(BaseItemSimilarityMatrixRecommender):
    """ ItemKNNScoresHybridTwoRecommender
...
class MergeTwoModelsByScoresLinear(BaseItemSimilarityMatrixRecommender):
    """ ItemKNNScoresHybridTwoRecommender
    Hybrid of two prediction scores R = R1*alpha + R2*beta
    """

    RECOMMENDER_NAME = "MergeTwoModelsByScoreLinear"

    def __init__(self, URM_train, recommenders: list):
        super(MergeTwoModelsByScoresLinear, self).__init__(URM_train)
